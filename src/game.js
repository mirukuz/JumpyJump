...
const palette = ['#c7eae4', '#a7e8bd', '#fcbcb8', '#efa7a7', '#ffd972']

const hitSound = new Audio('/sounds/quak2.mp3')
const fallSound = new Audio('/sounds/ah.mp3')
...
class Game {
	...
	_handleMouseDown() {
		...
		if (!this.jumperStat.ready && this.jumper.scale.y > 0.02) {
			increment ++
			this.jumper.scale.y -= 1/(80^increment);
			this.jumper.rotation.y = this.cubeStat.nextDir == 'left' ? -Math.PI/2 : Math.PI 
	...
	_createJumper() {
		...
		gltfLoader.load(
			...
			(gltf) =>
			{
				this.jumper = gltf.scene.children[0];
				this.jumper.position.y = 1;
	...
	_updateCamera() {
		...
		if(cur.x < next.x) {
			this.cameraPros.current.x += 0.25;
		}
		if(cur.x > next.x) {
			this.cameraPros.current.x -= 0.25;
		}
		if(cur.z < next.z) {
			this.cameraPros.current.z += 0.25;
		}
		if(cur.z > next.z) {
			this.cameraPros.current.z -= 0.25;
		}
		if (Math.abs(this.cameraPros.current.z - this.cameraPros.next.z) < 0.05) {
			this.cameraPros.current.z = this.cameraPros.next.z;
		} else if (Math.abs(this.cameraPros.current.x - this.cameraPros.next.x) < 0.5) {
